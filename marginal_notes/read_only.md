...
#### Введение
Сложно переоценить значение концепций переменной и константы в языках программирования. Эти две кнцепции на столько важны, что по тому как в языке реализована их поддержка, язык будет отнесен к одной из трех групп: те в которых нет констант, те в которых нет переменных, и те в которых эти две концепции поддерживаются одновременно. Примером языков первой группы являлся js, до ECMAScript 2015. Ко второй группе относятся функциональные языки, такие как F#. Но большенство языков, все же, пытаются дать программистам возможность использовать обе концепции.

Что такое константность зависит от контектса. То что поддерживается в языках программирования - это, скажем так, элементарная константность, т.е. неизменность данных в некоторой обласит памяти, обеспечиваемая на уровне исполняемого кода. Когда мы пишем const, то язык программирования гарантирует, что попытка изменить данный в области памяти помеченной как const приведут к ошибке.

Но константность может быть и более выского уровня, например, незименность данных содержащихся в событии, в event-driven системах. Но даже константность более высокого уровня, в конечном итоге, будет выражена в конструкциях конкретного языка программирования.   
