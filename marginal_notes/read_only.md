...
##### Неизменяемые объекты, ключевое слово readonly применительно к структурам, records.
Следующий уровень асбтракции в выражении незименности, являются так называемые неизменяемые объекты[<sup>15</sup>](#15). Неизменяемый объект - это объект, состояние которого задается на момент создания, и не изменяется в течении всего жизненного цикла. К такому типу объектов часто относят объекты-значения[<sup>14</sup>](#14), например в C# это объекты типа DateTime или String. Все операции, которые могли бы привести к изменению состояния, всегда создают новый экземпляр с новым значением, изменить значения исходного объекта невозможно. Ниже приведен код деманстрирующий этот факт: 
```C#
var a = new DateTime(2112, 1,21);
var b = a.AddDays(11);
	
Console.WriteLine(Object.ReferenceEquals(a, b)); // False -> а и b - это разные объекты.
Console.WriteLine(a); // 2112-01-21 -> значение a не изменилось.
Console.WriteLine(b); // 2012-02-01 -> в b было сохранено новое значение.

```

Типы неизменяемых объектов могут быть определены пользователем:
```C#
struct Event // аналогично можно и для class.
{
...
```C#
struct NotConst // аналогично и для class.
{
...
```
При каждом обращении к полю Date у объекта этого типа, клиент будет получать новое значение - объект не является иммутабельным, даже имея свойство "только для чтения", даже если бы был помечен как `readonly struct`.

Открытые `readonly` поля, в качестве способа получения доступа к неизменяемому состоянию, явлюятся более надежным признаком константости, чем свойства и, тем более, методы, если смотреть с позиций клиента кода.

Надо отметить, что для упрощения определения типов неизменняемых объектов в C# v8 предлагается использовать упрощенный синтаксис, т.н. `Records`[<sup>17</sup>](#17).

Выше в примерах кода указано, что вместо `struct` можно использовать `class` в определениях типов неизменяемых объектов. *Это так, но использование структуры дает дополнительный уровень защиты состояния. Если неизменяемый объект содержит поля только элементарных типов, или типов неизменяемых объектов, то автор кода, используя структуру может доплонительно указать, что это константа. При передаче ее в качестве аргумента (без дополнительных "обвязок") или как возвращаемого значения (так же без дополнительных "обвязок") мы всегда передаем/возвращаем копию, а не сам объект. Это дополнительный фактор удобен, если мы хотим передать под контроль "машине" обеспечение еще большего уровня константонсти: если мы не можем *

##### Константность и вопросы использования памяти, параметры in.
...
<a class='anchor' id='17'>17</a>. <a href="https://github.com/dotnet/csharplang/blob/master/proposals/records.md#examples"> (dotnet/csharplang) records</a>   
<a class='anchor' id='18'>18</a>. <a href="https://docs.microsoft.com/en-us/dotnet/csharp/write-safe-efficient-code">Declare readonly structs for immutable value types</a> 
