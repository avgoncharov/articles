...
```
##### const и readonly
Для объявления некоторых данных как "константы" в C# есть два ключивых слова: `const` и `readonly`.
Ключевое слово `const`[<sup>12, </sup>](#12) позволяет делать именованные константы. В С# оно применимо только к элементарным типам, строкам и перечислениям. Фактически, после компиляции, значение сохраненное в константе подставляется в места использования этой константы. Т.е. в итоге это идентично использованию литерала.
