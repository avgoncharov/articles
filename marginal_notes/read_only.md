...
Первое из них то, что описано выше, этим ключевым словом можно помечать поля, которые не должны менять свое значение. Это могут быть поля любих типов, и элементарных, и типов-значения, и ссылочных. Кроме того, поля помеченные как `readonly` могут быть как статическими, так и не статическими. Смысл ключевого слова `readonly`, применительно к полям, зависимости от типа поля. Так, если речь идет о статических полях элементарного типа (или строки), то `readonly` можно воспринимать в исходном коде как синоним `const`. Но если речь идет полях - типах-значениях или полях ссылочного типа, то значение `readonly` полностью определяется внутренним содержанием типа, и контекстом.
